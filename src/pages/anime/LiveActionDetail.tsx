...
const LiveActionDetail = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['live-action-detail', slug],
    queryFn: () => animeApi.getLiveActionDetail(slug!),
    enabled: !!slug,
  });
  ...
  if (!detail) {
    return (
      <div className="container mx-auto px-4 py-8 text-center">
        <p className="text-muted-foreground">Live action not found</p>
        <Link to="/anime/live-action">
          <Button className="mt-4">Back to Live Action</Button>
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative h-64 md:h-80 overflow-hidden">
        ...
        <Link to="/anime/live-action" className="absolute top-4 left-4">
